...
export default function PollComponent({ poll, onVote }: PollProps) {
  ...
  return (
    <div className={`mt-3 rounded-2xl border overflow-hidden ${tc.border} ${tc.bgCard}`}>
      <p className={`px-4 pt-3 pb-2 font-bold text-[15px] ${tc.text}`}>{poll.question}</p>
        ...
        {poll.options.map((option) => (
          <button
            ...
            className={`w-full relative rounded-lg overflow-hidden transition-all ${
              poll.hasVoted
                ? 'cursor-default'
                : `${tc.bgHoverSecondary} cursor-pointer`
            } ${tc.bgTertiary}`}
          >
            {/* Progress bar background */}
            {poll.hasVoted && (
              <div
                className={`absolute inset-0 ${option.voted ? 'bg-blue-500/20' : tc.bgSecondary} transition-all duration-500`}
            ...
            <div className="relative flex items-center justify-between px-4 py-2.5">
              <div className="flex items-center gap-2">
                ...
                <span className={`text-[15px] ${tc.text}`}>{option.text}</span>
              ...
              {poll.hasVoted && (
                <span className={`text-sm font-bold ${option.voted ? 'text-blue-400' : tc.textMuted}`}>
                  {option.percentage}%
                </span>
              )}
            </div>
