    ...
    >
      ...
      <div className="flex gap-3">
        ...
        <div className="flex-shrink-0">
          <div
            ...
            className="w-10 h-10 rounded-full bg-gradient-to-br from-blue-500 to-purple-600 flex items-center justify-center text-lg hover:opacity-80 transition-opacity"
          >
            {post.user.avatar}
          </div>
        </div>
        ...
        <div className="flex-1 min-w-0">
          ...
          {showActions && (
            <div
              className="absolute right-4 mt-1 w-64 bg-black border border-gray-800 rounded-xl shadow-xl z-50 py-2"
              ...
              {isOwnPost ? (
                <>
                  <button
                    ...
                    className="w-full flex items-center gap-3 px-4 py-3 hover:bg-gray-900 text-left text-[15px] text-white"
                  >
                  ...
                  <button
                    ...
                    className="w-full flex items-center gap-3 px-4 py-3 hover:bg-gray-900 text-left text-[15px] text-red-500"
                  >
                    <svg viewBox="0 0 24 24" className="w-5 h-5" fill="currentColor">
                ...
                <>
                  <button className="w-full flex items-center gap-3 px-4 py-3 hover:bg-gray-900 text-left text-[15px] text-white">
                    <svg viewBox="0 0 24 24" className="w-5 h-5" fill="currentColor">
            ...
            <div className="mt-3 flex gap-3" onClick={(e) => e.stopPropagation()}>
              ...
              <div className="flex-1">
                <textarea
                  ...
                  className="w-full bg-transparent text-[15px] text-white placeholder-gray-500 outline-none resize-none min-h-[40px]"
                  autoFocus
                />
      ...
      {showShareModal && (
        <div className="fixed inset-0 z-[100] flex items-end sm:items-center justify-center" onClick={(e) => { e.stopPropagation(); setShowShareModal(false); }}>
          ...
          <div className="relative w-full max-w-[400px] mx-4 bg-gray-900 rounded-2xl border border-gray-800 p-4 mb-4 sm:mb-0" onClick={(e) => e.stopPropagation()}>
            <h3 className="text-xl font-bold text-white mb-4">Share post</h3>
            <div className="space-y-1">
              <button onClick={() => { handleCopyLink(); }} className="w-full flex items-center gap-4 px-3 py-3 rounded-lg hover:bg-gray-800 transition-colors">
                <div className="w-10 h-10 rounded-full bg-gray-800 flex items-center justify-center">
                  <svg viewBox="0 0 24 24" className="w-5 h-5 text-white" fill="currentColor">
                    <path d="M16 1H4c-1.1 0-2 .9-2 2v14h2V3h12V1zm3 4H8c-1.1 0-2 .9-2 2v14c0 1.1.9 2 2 2h11c1.1 0 2-.9 2-2V7c0-1.1-.9-2-2-2zm0 16H8V7h11v14z"/>
                  </svg>
                </div>
                <span className="text-white text-[15px]">{copied ? 'Copied!' : 'Copy link'}</span>
              </button>
              <button onClick={handleShareNative} className="w-full flex items-center gap-4 px-3 py-3 rounded-lg hover:bg-gray-800 transition-colors">
                <div className="w-10 h-10 rounded-full bg-gray-800 flex items-center justify-center">
                  <svg viewBox="0 0 24 24" className="w-5 h-5 text-white" fill="currentColor">
                    <path d="M12 2.59l5.7 5.7-1.41 1.42L13 6.41V16h-2V6.41l-3.3 3.3-1.41-1.42L12 2.59zM21 15l-.02 3.51c0 1.38-1.12 2.49-2.5 2.49H5.5C4.11 21 3 19.88 3 18.5V15h2v3.5c0 .28.22.5.5.5h12.98c.28 0 .5-.22.5-.5L19 15h2z"/>
                  </svg>
                </div>
                <span className="text-white text-[15px]">Share via...</span>
              </button>
              <button onClick={() => { setShowShareModal(false); setShowReplyBox(true); }} className="w-full flex items-center gap-4 px-3 py-3 rounded-lg hover:bg-gray-800 transition-colors">
                <div className="w-10 h-10 rounded-full bg-gray-800 flex items-center justify-center">
                  <svg viewBox="0 0 24 24" className="w-5 h-5 text-white" fill="currentColor">
                    <path d="M1.751 10c0-4.42 3.584-8 8.005-8h4.366c4.49 0 8.129 3.64 8.129 8.13 0 2.96-1.607 5.68-4.196 7.11l-8.054 4.46v-3.69h-.067c-4.49.1-8.183-3.51-8.183-8.01z"/>
                  </svg>
                </div>
                <span className="text-white text-[15px]">Quote post</span>
              </button>
              <button onClick={() => { onBookmark(post.id); setShowShareModal(false); }} className="w-full flex items-center gap-4 px-3 py-3 rounded-lg hover:bg-gray-800 transition-colors">
                <div className="w-10 h-10 rounded-full bg-gray-800 flex items-center justify-center">
                  <svg viewBox="0 0 24 24" className="w-5 h-5 text-white" fill="currentColor">
                    <path d="M4 4.5C4 3.12 5.119 2 6.5 2h11C18.881 2 20 3.12 20 4.5v18.44l-8-5.71-8 5.71V4.5z"/>
                  </svg>
                </div>
                <span className="text-white text-[15px]">{post.bookmarked ? 'Remove bookmark' : 'Bookmark'}</span>
              </button>
              <button onClick={() => { onLike(post.id); setShowShareModal(false); }} className="w-full flex items-center gap-4 px-3 py-3 rounded-lg hover:bg-gray-800 transition-colors">
                <div className="w-10 h-10 rounded-full bg-gray-800 flex items-center justify-center">
                ...
                <span className="text-white text-[15px]">{post.liked ? 'Unlike' : 'Like'}</span>
              </button>
              <button onClick={() => { onRetweet(post.id); setShowShareModal(false); }} className="w-full flex items-center gap-4 px-3 py-3 rounded-lg hover:bg-gray-800 transition-colors">
                <div className="w-10 h-10 rounded-full bg-gray-800 flex items-center justify-center">
                  <svg viewBox="0 0 24 24" className="w-5 h-5 text-green-500" fill="currentColor">
                    <path d="M4.75 16.5h10.5v-3.25l4.5 4-4.5 4V18H4.75v-1.5zm14.5-9H8.75v3.25l-4.5-4 4.5-4V5.25h10.5v1.5z"/>
                  </svg>
                </div>
                <span className="text-white text-[15px]">{post.retweeted ? 'Undo repost' : 'Repost'}</span>
              </button>
            </div>
          </div>
        </div>
