...
export default function Sidebar({ currentPage, onNavigate, onCompose, unreadNotifications, unreadMessages }: SidebarProps) {
  return (
    <aside className="fixed left-0 top-0 h-full w-[68px] xl:w-[275px] flex flex-col items-center xl:items-start px-2 xl:px-3 py-3 border-r border-gray-800/50 z-50 bg-black/80 backdrop-blur-xl">
      {/* Logo */}
      <div className="p-3 mb-1 hover:bg-gray-800/50 rounded-full cursor-pointer transition-colors">
        <XLogo />
      </div>
          ...
          return (
            <button
              ...
              className={`flex items-center gap-4 px-3 py-3 rounded-full transition-all duration-200 group w-full relative
                ${isActive ? 'font-bold text-white' : 'text-gray-300 hover:text-white hover:bg-gray-800/40'}`}
            >
      ...
      <div className="mt-auto w-full">
        <button className="flex items-center gap-3 p-3 rounded-full hover:bg-gray-800/50 transition-colors w-full">
          ...
          <div className="hidden xl:block flex-1 text-left min-w-0">
            <div className="flex items-center gap-1">
              <span className="font-bold text-[15px] text-white truncate">{currentUser.name}</span>
          ...
          <span className="hidden xl:block text-gray-500"><MoreHorizontal /></span>
        </button>
      </div>
    </aside>
  );
